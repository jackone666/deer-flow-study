...
```mermaid
flowchart TD
    A[make_lead_agent 入口] --> B[1. 提取运行时参数]
    B --> C[2. 加载 Agent 配置 + 技能白名单]
    C --> D[3. 模型选择：四级回退]
    D --> E[4. 注入追踪回调和运行元数据]
    E --> F[5. 加载技能用于工具策略过滤]
    F --> G{6. 分支判断}
    G -->|Bootstrap 模式| H[6a. 精简 prompt + 仅 setup_agent 工具]
    G -->|常规模式| I[6b. 完整配置 + update_agent 工具]
    H --> J[7. 按技能 allowed-tools 白名单过滤]
    I --> J
    J --> K[8. MCP 工具分离到延迟集合]
    K --> L[9. 构建中间件链 + 渲染 prompt + create_agent]
    L --> M[返回编译后的 StateGraph]
```
