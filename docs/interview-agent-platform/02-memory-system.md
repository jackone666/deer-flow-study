...
## 面试补强：记忆系统要讲“存什么、怎么存、怎么取、怎么纠偏”

面试官问记忆时，最忌讳只说“短期记忆 + 长期记忆”。要把写入门禁、读取策略和冲突处理讲清楚。

### 标准回答骨架

> 记忆系统解决的是跨会话个性化和用户纠偏延续，不是保存聊天记录。我把它拆成写入、存储、读取和纠偏四步：写入时只抽取稳定偏好、身份事实、长期项目背景和明确纠偏；存储时按 user / agent / thread scope 隔离，并保留来源、时间和置信度；读取时按当前任务相关性和 token 预算注入；如果用户纠正旧事实，不能简单追加，要删除或降权冲突事实。

### 面试官追问：短期记忆、长期记忆、摘要有什么区别？

| 类型 | 服务对象 | 保存内容 | 生命周期 |
| --- | --- | --- | --- |
| 近期消息 | 当前连续对话 | 最近用户目标、追问、工具结果 | 当前上下文窗口 |
| 摘要 | 当前线程续接 | 任务进度、决策、失败尝试、待办 | 当前 thread |
| 长期记忆 | 跨会话个性化 | 偏好、稳定事实、明确纠偏 | user / agent scope |

推荐话术：

> 摘要是为了当前任务不断线，长期记忆是为了未来会话更懂用户。摘要可以包含临时任务路径，但长期记忆不应该记一次性路径。

### 面试官追问：为什么不每 30 秒更新一次？

> 过于频繁会增加模型调用成本，也容易把临时状态写成长期事实。我更倾向于事件触发 + 去抖合并：当一轮任务结束、用户明确纠偏、出现强化信号，或者发生可复用工作流时再入队。短时间内多次更新按 thread/user/agent 合并，避免重复写入。

如果被问“用户上午问一次、下午问一次是不是会启动两次”，可以答：

> 可以启动两次，但写入要经过门禁。不是每次 query 都产生长期记忆，只有识别到稳定偏好、纠偏或长期项目事实才写。否则只是当前线程上下文，不进长期记忆。
...
```text
写入前：只接受高价值候选，过滤临时路径、敏感信息、模型猜测
写入时：带 scope、source、confidence、created_at
写入后：冲突检测，纠偏事实优先，旧事实 tombstone 或降权
读取时：按相关性、时间、置信度和预算筛选
治理上：记录 correction_repeat_rate 和 stale_memory_detected
```

### Ownership 口径

> 我负责的是记忆链路里的抽取规则、异步入队、去抖合并、纠偏信号识别和冲突处理策略；底层存储可以是文件、Redis 或数据库，但关键是记忆不是聊天日志，而是有 scope、置信度和门禁的长期事实系统。
