...
## 面试补强：摘要压缩要讲预算、保护字段和缓存取舍

这题不要只答“超过 token 阈值就摘要”。更好的说法是：

> 我把摘要压缩看成线程状态的压缩视图，不是普通聊天总结。目标是在不破坏任务连续性的前提下，把旧历史从模型窗口里移出去，同时保留目标、硬约束、关键决策、工具结果、产物路径和未完成事项。触发上用 token budget，而不是固定轮数；执行上要保护动态提醒和未闭合 tool pair；评估上不能只看压缩率，要看摘要后任务是否还能继续完成。

### 回答骨架

```text
为什么做：
  长线程会带来成本、延迟、注意力污染和窗口溢出。

什么时候做：
  token 接近预算阈值时触发，并给模型输出、工具 schema、动态上下文留 buffer。

压什么：
  旧探索过程、重复日志、低价值中间输出。

保什么：
  用户目标、硬约束、关键决策、文件路径、错误结论、当前 todo、动态提醒、未闭合 tool call。

怎么验：
  用长对话回归集比较摘要前后任务成功率、约束遵守率、工具选择一致性和 token 成本。
```
...
> 大窗口只是把问题往后推，不能解决成本、延迟和注意力污染。Agent 线程里很多内容是工具日志、失败尝试和重复上下文，这些内容即使能塞进去，也会干扰模型决策。摘要压缩本质上是上下文治理，不只是为了不超窗。

### 高频追问：摘要和 Prompt Cache 会不会冲突

不会，关键是把上下文分层：

| 层 | 是否适合缓存 | 说明 |
| --- | --- | --- |
| System prompt / tool policy | 适合 | 稳定层，尽量固定顺序和内容 |
| Skill / project instructions | 适合 | 版本变化才更新 |
| Summary | 半稳定 | 一段时间内复用，触发重摘要后变更 |
| Recent messages / tool result | 不适合 | 每轮变化 |

面试回答：

> 我会把稳定系统指令、工具策略和技能说明放在 cache-friendly 的前缀里，把摘要和最近消息放在后面的动态区。摘要本身会变，但它能减少动态区体积；Prompt Cache 负责降低稳定前缀成本，两者不是替代关系。

### 高频追问：怎么保证摘要不丢关键信息

不要说“靠 prompt 写清楚”。要说三层：

1. **结构化摘要模板**：强制输出 `Goal / Constraints / Decisions / Artifacts / Errors / Todos`。
2. **保护字段规则提取**：路径、commit、issue、配置名、用户硬约束这类精确字段优先规则保留。
3. **回归评估**：同一批长线程跑“无摘要 baseline”和“摘要后继续执行”，比较任务是否能完成。

关键指标：

| 指标 | 说明 |
| --- | --- |
| `summary_compression_ratio` | 压缩比例 |
| `constraint_preservation_rate` | 硬约束保留率 |
| `artifact_reference_preservation_rate` | 文件路径、产物引用保留率 |
| `post_summary_task_success_rate` | 摘要后任务成功率 |
| `summary_recovery_success_rate` | 用户纠错或重建摘要后的恢复率 |
| `prompt_cache_hit_rate` | 稳定前缀缓存命中率 |
| `ttft_ms` | 首 token 延迟 |
...
> 我主要负责摘要触发策略、cutoff 策略、动态提醒保护、tool pair 保护、摘要模板和 trace/eval 指标。底层模型可以替换，但什么时候压、压到哪里、哪些字段必须保留、摘要后怎么重建消息状态，是我负责设计和落地的工程边界。
