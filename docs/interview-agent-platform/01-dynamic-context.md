...
## 面试补强：上下文治理要讲 trade-off，不要只讲“分层”

你在面试里容易说很多层，但面试官真正想听的是为什么这么分、每层解决什么风险、如何证明有效。

### 标准回答骨架

> 上下文治理的核心不是把更多信息塞给模型，而是在 token 预算内给模型最有用、最可信、最新的信息。我会把上下文分成稳定规则、动态运行时信息、长期记忆、摘要和最近消息几层：稳定规则放 system prompt；日期、系统提醒、当前用户记忆在 before_model 动态注入；长历史通过摘要保留任务状态；最近消息原样保留。这样做的 trade-off 是实现更复杂，但可以减少上下文污染、降低 token 成本，也更容易观测每层上下文的来源和效果。

### 面试官追问：为什么不用完整历史 + 大窗口模型？

> 大窗口能缓解 token 限制，但不能解决污染和成本问题。完整历史里有过期信息、临时文件、失败尝试、外部网页内容和用户纠偏前的错误事实，全部塞进去会让模型更难判断优先级。上下文治理不是只为省 token，也是为了控制信息来源和优先级。
...
| 维度 | 判断方式 |
| --- | --- |
| 相关性 | 和当前 query / task / tool 目标是否相关 |
| 时效性 | 最近输入和明确纠偏优先于旧记忆 |
| 可信度 | 服务端规则 > 用户明确表达 > 外部资料 > 模型推断 |
| 成本 | token 占用是否值得，是否会挤掉最近消息 |

推荐话术：

> 我的排序是：安全和系统规则最高，用户当前明确输入其次，和当前任务相关的记忆再其次，泛化偏好最后。外部资料或模型推断不能伪装成系统事实。

### 面试官追问：上下文污染怎么发现？

> 不能只靠感觉。需要记录每次注入的 memory fact、summary id、dynamic context token 和回答是否引用了对应信息。如果用户纠偏说“你怎么又拿旧信息回答”，就沿着 trace 看旧事实是否被注入、是否被摘要保留、是否和当前输入冲突。指标上可以看 stale memory detected、memory hit rate、context token ratio 和 user correction rate。

### Ownership 口径

> 我负责的是上下文进入模型前的选择和注入机制，包括分层、预算、幂等和 trace；长期记忆抽取和摘要生成可以是其它模块，但我需要保证进入模型窗口的内容来源明确、优先级明确、可回溯。
