...
creating -> failed
executing -> timeout
idle -> orphan_detected -> cleanup
```

面试表达：

> 沙箱生命周期要和 run/thread 生命周期解耦。run 可以结束，但 thread 的 sandbox 可能保留一段时间，方便下一轮复用；超过 idle 时间再回收。

## 深挖补充：为什么不能静默 fallback

沙箱 fallback 是安全系统里的高风险点。

| fallback 行为 | 风险 |
| --- | --- |
| 远程不可用就本地执行 | 命令可能跑到宿主机 |
| provisioner_url 缺失仍启动 | 部署错误被隐藏 |
| Docker 不可用就跳过隔离 | 安全边界消失 |
| 路径映射失败仍继续 | 文件可能写到错误位置 |

推荐回答：

> 安全边界不能静默降级。沙箱不可用时应该 fail-fast，让部署问题暴露出来，而不是换一条更危险的执行路径。

## 深挖补充：路径隔离怎么讲

Agent 文件执行最容易出问题的是路径。
...
1. 工具只能访问 sandbox 映射后的 workspace。
2. 用户上传和模型输出分目录。
3. 返回给前端的是虚拟路径或 artifact id，不暴露宿主机真实路径。
4. `..`、绝对路径、符号链接跳出 workspace 都要拦截。

高分表达：

> 沙箱不仅是进程隔离，还包括路径语义隔离。模型不应该知道宿主机路径，只能操作线程级 workspace。
...
| 资源 | 策略 |
| --- | --- |
| CPU | 单 sandbox 配额，防止死循环 |
| 内存 | OOM 限制和错误标准化 |
| 磁盘 | workspace 大小上限和清理 |
| 网络 | 默认限制或域名 allowlist |
| 进程 | 限制 fork bomb |
| 时间 | 命令级 timeout |

如果面试官问“为什么本地开发没全做”，可以答：

> 本地开发可以先实现接口抽象和远程 backend，生产化再加资源限额。关键是架构上不能把命令执行绑死在宿主机。
...
| `sandbox_id` | 关联同一执行环境 |
| `thread_id` | 判断是否正确复用 |
| `sandbox_action` | acquire / discover / create / execute / cleanup |
| `cold_start_ms` | 衡量创建成本 |
| `execute_ms` | 单次命令耗时 |
| `exit_code` | 命令执行结果 |
| `timeout` | 是否超时 |
| `audit_decision` | SandboxAudit 是否拒绝 |
| `cleanup_reason` | idle、error、manual |
...
这样模型能继续推理，前端也能展示产物，观测系统能统计执行成本。

## 深挖补充：面试攻防

### Q：沙箱能不能完全保证安全？

不能。沙箱是重要隔离层，但还要配合工具权限、Guardrails、路径校验、资源限额和审计。安全是分层体系，不是单点能力。

### Q：为什么按 thread_id 复用，不按 run_id？

run_id 太短，一次任务结束后下一轮会丢掉环境；thread_id 更符合用户连续任务。为了避免资源泄漏，再配 idle timeout 和 cleanup。

### Q：如果两个 run 同时操作同一个 thread sandbox 怎么办？

需要线程级锁或命令队列，至少对写文件和 bash 执行做互斥。否则并发写入会导致文件状态不确定。

### Q：远程沙箱比本地慢，值不值？

值。远程化带来隔离、资源管理、审计和多租户能力。性能可以通过懒加载、warm pool、镜像预热和依赖缓存优化，但安全边界不能靠本地裸执行换速度。

## 面试补强：沙箱题要把“权限、策略、执行”分清

沙箱很容易被答成“我们有 Docker”。面试官真正关心的是多租户隔离、路径边界、资源回收、失败策略和可观测性。

### 标准回答骨架

> 在我的 Agent Harness 里，沙箱不是普通工具，而是受控执行环境。工具权限决定模型有没有入口，Guardrails 决定这次调用允不允许，Sandbox 决定允许后在哪里执行，SandboxAudit 决定具体 bash 内容是否危险。这样即使模型被诱导调用危险命令，也不会直接落到宿主机。

### 面试官追问：为什么不在本地执行，省得慢？

> 本地执行会破坏多租户和宿主机安全边界。Agent 能写文件、跑命令、访问环境变量，如果直接在宿主机执行，prompt injection 或工具误调用会变成真实安全事故。远程沙箱慢一点，但换来路径隔离、资源限额、审计和回收能力。性能应该通过懒加载、warm pool、镜像预热和依赖缓存优化，而不是牺牲安全边界。

### 面试官追问：沙箱怎么和用户隔离关联？

推荐回答：

> 权限边界必须至少绑定 user_id 和 thread_id。user_id 决定谁有权访问这个 sandbox，thread_id 决定同一连续任务能复用 workspace。只用 thread_id 不够，因为 thread id 泄露或接口越权时可能访问别人的文件；只用 run_id 又太短，会丢掉多轮任务的工作区。
...
```text
普通网络超时：
  可有限重试，记录 retry_count

命令级超时：
  终止当前命令，返回标准化 ToolMessage error

sandbox acquire 超时：
  记录 provider / provisioner 错误，不 fallback 到本地

多次失败：
  让模型重新规划，或提示用户当前执行环境不可用
```

推荐话术：

> 超时不能简单吞掉。工具结果要返回结构化错误，让模型知道是网络、命令、资源还是沙箱启动失败；同时 trace 里记录 sandbox_id、action、duration、exit_code 和 timeout，便于定位。

### 面试官追问：你负责沙箱哪部分？

> 我负责把沙箱接入 Agent 工具执行链路，包括懒加载、thread 级复用、路径约束、工具结果结构化和 trace 字段；底层容器或 provisioner 可以由基础设施团队提供，但 Harness 层必须保证工具不会绕过沙箱执行。
